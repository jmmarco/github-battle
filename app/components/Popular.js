  ...
  return (
    <ul className="grid space-around">
      {repos.map((repo, index) => {
        const { owner, html_url, stargazers_count, forks, open_issues } =
          repo;
...
function reposReducer(state, action) {
  console.log('does this even fire?', action);
  switch (action.type) {
    case 'ERROR':
      return {
        ...state,
...
export default function Popular() {
  ...
  useEffect(() => {
    ...
    fetchPopularRepos(selectedLanguage)
      ...
      .catch((err) => {
        ...
        dispatch({
          type: 'ERROR',
          payload: `There was an error fetching the repositories. ${err}}`,
        });
      })
  ...
  return (
    <React.Fragment>
      ...
      {/* <div>{JSON.stringify(repos, 2, null)}</div> */}
